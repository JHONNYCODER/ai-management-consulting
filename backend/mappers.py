...
def map_state_to_api_response(state: Dict) -> AnalyticsResponse:
    ...
    # 1. Remove internal objects that cannot/should not be serialized or sanitized
    internal_keys = ["df", "pipeline_cache", "config"]
    clean_state = {k: v for k, v in state.items() if k not in internal_keys}
    
    # 2. Sanitize NaN/Inf values to prevent Pydantic validation errors
    safe_state = sanitize_nan_values(clean_state)
    
    # 3. Summary
    summary = Summary(
        rows=safe_get(safe_state, "rows", 0),
        columns=safe_get(safe_state, "columns", 0)
    )
    
    # 4. Profile
    raw_profile = safe_get(safe_state, "profile", {})
    profile = {
    ...
    # 5. Correlations
    raw_pairs = safe_get(safe_get(safe_state, "correlations", {}), "pairs", [])
    correlations = {
        "pairs": [CorrelationPair(**p) for p in raw_pairs if isinstance(p, dict)]
    }
    
    # 6. Health
    raw_health = safe_get(safe_state, "dataset_health", {})
    ...
    raw_exec = safe_get(safe_state, "executive_synthesis", {})
    ...
    # 8. Clean Analytical Stability
    raw_stability = safe_get(safe_state, "analytical_stability", {})
    clean_stability = {
    ...
    # 9. Clean Contextual Synthesis (Filter empty themes)
    raw_contextual = safe_get(safe_state, "contextual_synthesis", {})
    ...
    # 10. Clean Theme Metrics (Filter empty themes)
    raw_tm = safe_get(safe_state, "theme_metrics", {})
    clean_tm = {
    ...
    # 11. Clean Cross Theme Reasoning (Fix Ghost Themes)
    raw_ctr = safe_get(safe_state, "cross_theme_reasoning", {})
    ...
    # 12. Clean Recommendations (Strip driver_id)
    raw_recs = safe_get(safe_state, "recommendations", {})
    ...
    file_name = os.path.basename(safe_get(safe_state, "file_path", "unknown.csv"))
    ...
    data = AnalyticsResponseData(
        ...
        anomaly_details=safe_get(safe_state, "anomaly_details", []),
        dataset_health=dataset_health,
        analytical_stability=clean_stability,
        conflicts=safe_get(safe_state, "conflicts", []),
        contextual_synthesis=clean_contextual,
        cross_theme_reasoning=clean_ctr,
        narrative_summary=safe_get(safe_state, "narrative_summary", {}),
        final_insights=safe_get(safe_state, "final_insights", {}),
        executive_synthesis=executive_synthesis,
        recommendations=clean_recs,
        chart_url=safe_get(safe_state, "chart_url"),
        chart_path=safe_get(safe_state, "chart_file"),
        charts=safe_get(safe_state, "charts")
    )
    
    # 14. Wrap in top-level response
    success = len(safe_get(safe_state, "metadata", {}).get("layers_failed", [])) == 0
